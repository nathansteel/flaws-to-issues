...
function createVeracodeFlawID(flaw) {
    // [VID:FlawID]
    return('[VID:' + flaw.finding_details.component_id + ']')
}
...
async function getAllVeracodeIssues(options) {
    ...
    for(const element of label.flawLabels) {
        ...
        let uriType = encodeURIComponent(label.otherLabels.find( val => val.id === 'sca').name);
...
async function processSCAFlaws(options, flawData) {
    ...
    for( index=0; index < flawData._embedded.findings.length; index++) {
        ...
        console.debug(`processing flaw ${flaw.finding_details.component_id}, VeracodeID: ${vid}`);
        ...
        let title = `${flaw.finding_details.cve.name} ` + createVeracodeFlawID(flaw);
        let lableBase = label.otherLabels.find( val => val.id === 'sca').name;
        let severity = flaw.finding_details.severity;
        let bodyText = `**Component Name:** ${flaw.finding_details.component_filename}`;
        bodyText += `\n\n**Affected Version:** ${flaw.finding_details.version}`;
        bodyText += `\n\n**CVE:** <a href="${flaw.finding_details.cve.href}">${flaw.finding_details.cve.name}</a>`;
...
module.exports = { processSCAFlaws }